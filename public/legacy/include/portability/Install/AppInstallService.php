...
class AppInstallService
{
    ...
    protected function performSetup(): array
    {
        ...
        //custom code by Sawood starts - 25-05-26
        $level = ob_get_level();
        ob_start();
        try {
            $basePath = getcwd();
            $sourcePath = $basePath . '/install/rahbar_package';
            // $this->installModulePackage($sourcePath.'/Fyntriaging.zip');
            $zipPath = $sourcePath.'/Rahbaraddon.zip';
            register_shutdown_function(function () use ($zipPath) {
                try {
                    ...
                    $tempDir = 'cache/upgrades/temp/' . uniqid('rahbar_');
                    sugar_mkdir($tempDir, 0775, true);
                    $zip = new ZipArchive();
                    ...
                    $zip->close();
                    $manifest = [];
                    if (file_exists($tempDir . '/manifest.php')) {
                        include $tempDir . '/manifest.php';
                    }
                    if (empty($manifest)) {
                        return;
                    }
                    ...
                    $mi->silent = true;
                    $mi->install($tempDir, $manifest);
                    while (ob_get_level() > $level) {
                        ob_end_clean();
                    }
                } catch (\Throwable $e) {
                    file_put_contents(
                        'rahbar_module_install_error.log',
                        date('Y-m-d H:i:s') . "\n" .
                        $e->getMessage() . "\n" .
                        $e->getTraceAsString() . "\n\n",
                        FILE_APPEND
            ...
            'repairRelationships',
            'rebuildJsFiles',
            'rebuildDashlets'], [], true, false);
        } catch (\Throwable $e) {
            file_put_contents(
                'module_install_error.log',
        ...
        //custom code by Sawood ends - 25-05-26
        return [
            'success' => true
        ];
    }
...
///////////////////////////////////////////////////////////////////////////////
    ...
    //custom code by Sawood starts - 25-05-26
    protected function installModulePackage(string $zipFile): bool
    {
        if (!file_exists($zipFile)) {
            file_put_contents('rahbar_install_debug.log', "ZIP NOT FOUND: {$zipFile}\n", FILE_APPEND);
            return false;
        }
        require_once 'ModuleInstall/ModuleInstaller.php';
        require_once 'ModuleInstall/PackageManager/PackageManager.php';
        ...
        if ($zip->open($zipFile) !== true) {
            file_put_contents('rahbar_install_debug.log', "FAILED TO OPEN ZIP: {$zipFile}\n", FILE_APPEND);
            return false;
        }
        $zip->extractTo($tempDir);
        $zip->close();
